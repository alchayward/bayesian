...
def update_strengths(db,session_id):
    ...
    str_dict = bayes.update_results(sess,db['Games'])
...
def get_suggested_games(db,session_id,round):
    kl = bayes.update_kl_info(db['Sessions'][session_id],db['Games'])
    games = bayes.get_best_staging(db['Sessions'][session_id],db['Games'],kl,round)
